...
public class SkyblockItemCache {
    ...
    private final ExecutorService loader = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "JEF-ItemCache");
        t.setDaemon(true);
        return t;
    });
    @Getter
    ...
    private void loadSync() {
        try {
            ...
            loaded = true;
            System.out.println("[JEF] Loaded " + items + " items, " + skulls + " repo skulls + " + BUILTIN_SKULLS.size() + " builtin island skulls (" + skullsFailed + " failed) from NEU repo.");
        } catch (Exception e) {
